...
class Api:
    ...
    def get_settings(self):
        return {
            ...
            'auto_clear_json': self.settings.get('auto_clear_json', True),
            'bg_image_enabled': self.settings.get('bg_image_enabled', True),
            'bg_image_path': self.settings.get('bg_image_path', ''),
            'flag_density': self.settings.get('flag_density', 'normal'),
            'font_scale': self.settings.get('font_scale', 100),
            'accent_color': self.settings.get('accent_color', ''),
            'border_radius': self.settings.get('border_radius', 10),
            'glass_blur': self.settings.get('glass_blur', 20),
            'show_column_value': self.settings.get('show_column_value', True),
            'show_column_type': self.settings.get('show_column_type', True),
            'show_column_bind': self.settings.get('show_column_bind', True),
            'show_column_dot': self.settings.get('show_column_dot', True),
    ...
    def deapply_flags(self):
        """Revert applied FFlags in the running Roblox instance.

        Clears ClientAppSettings.json and writes each flag's original_value
        back to memory. Does NOT kill Roblox or remove flags from the editor.
        """
        ...
        try:
            ...
            if self.roblox_manager.is_attached and self.roblox_manager.open_process_for_write():
                ...
                for flag in flags_snapshot:
                    ...
                    addr_data = fresh_addrs.get(clean) or fresh_addrs.get(name)
                    if not addr_data:
                        skipped += 1
                        continue
                    ...
                    for addr_entry in addr_data:
                        ...
                        try:
                            ok, _ = self.roblox_manager.write_flag_at_address(flag_type, abs_addr, str(orig))
                            if ok:
                                reverted += 1
                                break
                        except Exception:
            ...
            log(f"[-] Deapply failed: {e}", (255, 100, 100))
    ...
    def save_custom_theme(self, theme_data):
        """Save a custom theme (CSS variable overrides) to disk."""
        try:
            import json
            from src.utils.paths import user_data_dir
            path = os.path.join(user_data_dir(), "custom_theme.json")
            if theme_data is None:
                # Remove custom theme file
                if os.path.exists(path):
                    os.remove(path)
                log("[-] Custom theme cleared", (180, 180, 180))
                return True
            theme_data['_active'] = True
            with open(path, 'w', encoding='utf-8') as f:
                json.dump(theme_data, f, indent=2)
            log("[+] Custom theme saved", (100, 255, 100))
            return True
        except Exception as e:
            log(f"[!] Save custom theme failed: {e}", (255, 100, 100))
            return False

    def load_custom_theme(self):
        """Load the saved custom theme from disk."""
        try:
            import json
            from src.utils.paths import user_data_dir
            path = os.path.join(user_data_dir(), "custom_theme.json")
    ...
    def save_setting(self, key, value):
        """Generic persistent setting save for UI customization."""
    ...
    def get_default_bg_path(self):
        """Return the absolute path to the default bundled background image."""
        from src.utils.helpers import get_resource_path
        for name in ('bg.jpg', 'default_bg.jpg'):
            p = os.path.join(os.path.dirname(get_resource_path('')), 'src', 'gui', 'ui', name)
            if os.path.exists(p):
                return p
            p2 = get_resource_path(os.path.join('src', 'gui', 'ui', name))
            if os.path.exists(p2):
                return p2
        return ''

    def load_bg_image(self):
        """Return a base64 data URL for the current background image setting.
        
        Checks custom path first, falls back to bundled default.
        This avoids WebView2 file:// URL restrictions entirely.
        """
        path = self.settings.get('bg_image_path')
        if path:
            url = self._read_bg_as_data_url(path)
            if url:
                return url
        return self._read_bg_as_data_url(self.get_default_bg_path())

    def _read_bg_as_data_url(self, filepath):
        """Read a background image file and return a base64 data URL.
        
        This bypasses WebView2's file:// URL restrictions.
        If filepath is None, uses the default bundled image.
        """
        import base64
        if not filepath:
            filepath = self.get_default_bg_path()
        ...
        try:
            ...
            log(f"[-] Failed to read bg image: {e}", (255, 100, 100))
            return None

    def set_bg_image_enabled(self, value):
        self.settings['bg_image_enabled'] = bool(value)
        Config.save_settings(self.settings)
        log(f"[+] Background image: {'ON' if value else 'OFF'}")

    def set_bg_image_path(self, path):
        if path:
            self.settings['bg_image_path'] = path
        else:
            self.settings.pop('bg_image_path', None)
        Config.save_settings(self.settings)

    def choose_bg_image(self):
        """Open file dialog to pick a background image, copy to user data dir."""
        if not self._window:
            return None
        try:
            ...
            if result and len(result) > 0:
                ...
                dst = os.path.join(user_data_dir(), f"custom_bg{ext}")
                shutil.copy2(src, dst)
                self.settings['bg_image_path'] = dst
                Config.save_settings(self.settings)
                log(f"[+] Background image set: {os.path.basename(src)}")
                return dst
            return None
        except Exception as e:
            log(f"[!] Failed to choose background image: {e}", (255, 100, 100))
            return None

    def load_css_file(self):
        """Open file dialog to pick a .css file and return its contents."""
        if not self._window:
            return None
        try:
            result = self._window.create_file_dialog(
            ...
            log(f"[!] Failed to load CSS file: {e}", (255, 100, 100))
            return None
    ...
    def bulk_edit_flags(self, names):
        """Open an input modal to set the same value for all selected flags."""
        if not self.flag_manager or not names:
            return {'ok': False}
        ...
        with self.flag_manager._lock:
            ...
            if not example:
                return {'ok': False}
            ftype = example.get('type', infer_type_from_name(names[0]) or 'string')
            if ftype != 'bool':
                return {'ok': False, 'needs_value': True, 'message': 'Enter value for all selected flags'}
            # For bool flags, just toggle all
            new_val = 'false' if str(example.get('value', 'true')).lower() == 'true' else 'true'
            for f in self.flag_manager.user_flags:
                if f['name'] in names:
                    f['value'] = new_val
        self.flag_manager.save_user_flags()
        log(f"[+] Bulk edited {len(names)} flags -> {new_val}")
        if self.settings.get('auto_apply'):
            self.inject()
        return {'ok': True}

    def bulk_set_enabled(self, names, enabled):
        """Enable or disable multiple flags at once."""
        if not self.flag_manager or not names:
            return
        count = 0
        with self.flag_manager._lock:
            for f in self.flag_manager.user_flags:
                if f['name'] in names:
                    ...
                    count += 1
        self.flag_manager.save_user_flags()
        log(f"[+] {'Enabled' if enabled else 'Disabled'} {count} flag(s)")
        if self.settings.get('auto_apply'):
            self.inject()
    ...
    def import_flags(self):
        ...
        try:
            ...
            if result and len(result) > 0:
                ...
                # Support both Bloxstrap-style dicts {"FFlagName": Value} and FFP list
