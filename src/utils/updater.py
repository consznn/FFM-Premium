...
def download_update(exe_url, new_version, progress_callback=None):
    ...
    try:
        ...
        downloaded = 0
        temp_setup = os.path.join(os.environ.get("TEMP", "."), f"Setup_FFP_{new_version}.exe")
        ...
        bat_path = os.path.join(os.environ.get("TEMP", "."), "ffp_update_launcher.bat")
...
def perform_silent_update(exe_url, new_version):
    ...
    try:
        ...
        # We must exit immediately so the installer can overwrite FFP.exe
