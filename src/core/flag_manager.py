...
class FlagManager:
    def __init__(self):
        ...
        self._deapplied = False  # When True, watchdog skips all enforcement
    ...
    def stop_watchdog(self):
        ...
        self._watchdog_running = False
        setattr(self, '_hotkey_running', False)
        if self._watchdog_thread and self._watchdog_thread.is_alive():
            self._watchdog_thread.join(timeout=1.0)
        hk_thread = getattr(self, '_hotkey_thread', None)
        if hk_thread and hk_thread.is_alive():
            hk_thread.join(timeout=1.0)
    ...
    def _watchdog_loop(self):
        ...
        while self._watchdog_running:
            ...
            if self._deapplied or not self.user_flags or not self._rm or not self._rm.is_attached:
                continue
    ...
    def apply_flags_hybrid(self, roblox_manager):
        self._deapplied = False  # Re-enable watchdog enforcement
        try:
            with self._lock:
                flags_snapshot = list(self.user_flags)
